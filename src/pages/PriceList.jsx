...
function NewAssemblyForm({ allPartNames, onSave, onCancel }) {
  const [data, setData] = useState({ name: "", description: "", category: "misc", labor_hours: "", quick_pick_category: "", quick_pick_unit: "", default_basis: "", components: [] });
      ...
      <div className="border border-accent/30 rounded-lg p-3 bg-accent/5 space-y-2">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          {isQuickPick && (
            <div className="col-span-2">
              <label className="text-xs text-muted-foreground font-medium">Basis <span className="font-normal">(applied to all components)</span></label>
              <select value={data.default_basis || ""} onChange={e => setData(p => ({ ...p, default_basis: e.target.value }))}
                className="mt-1 h-8 text-xs w-full border border-input rounded-md px-2 bg-transparent">
                <option value="">— select basis —</option>
                {FORMULA_FIELDS.map(f => <option key={f.field} value={f.field}>{f.label}</option>)}
              </select>
